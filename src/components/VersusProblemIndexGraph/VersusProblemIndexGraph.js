...
const VersusProblemIndexGraph = ({
  ...
  for (let i = 0; i < userSubmissions1.length; i++) {
    ...
    let key1 =
      userSubmissions1[i].contestId + userSubmissions1[i].problem.index;
    if (id1.has(key1)) {
      continue;
    }
    id1.set(key1, true);
  ...
  for (let i = 0; i < userSubmissions2.length; i++) {
    ...
    let key2 =
      userSubmissions2[i].contestId + userSubmissions2[i].problem.index;
    if (id2.has(key2)) {
      continue;
    }
    id2.set(key2, true);
